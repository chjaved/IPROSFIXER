// Consolidated: booking-photos, checklists, reschedule
...
function getTemplate(slug, category) {
  return CHECKLIST_TEMPLATES[SLUG_MAP[slug] || category] || CHECKLIST_TEMPLATES.cleaning
}
...
async function uploadPhoto(req, res, sql, userId, userType) {
  ...
  if (!booking_id || !photo_type || !photo_base64) return res.status(400).json({ success: false, message: 'booking_id, photo_type, and photo_base64 are required' })
  if (!['before', 'after'].includes(photo_type)) return res.status(400).json({ success: false, message: 'photo_type must be "before" or "after"' })
  const booking = await sql`SELECT * FROM bookings WHERE id=${booking_id}`
  if (!booking.length) return res.status(404).json({ success: false, message: 'Booking not found' })
  const b = booking[0]
  if (userType !== 'professional' || b.professional_id !== userId) return res.status(403).json({ success: false, message: 'Only assigned professional can upload photos' })
...
async function getChecklist(req, res, sql, checklistId, userId) {
  ...
  return res.json({ success: true, checklist: checklists[0], items })
}
...
async function createChecklist(req, res, sql, userId, userType) {
  ...
  if (!booking.length) return res.status(404).json({ success: false, message: 'Booking not found' })
  const b = booking[0]
  if (userType !== 'professional' || b.professional_id !== userId) return res.status(403).json({ success: false, message: 'Only assigned professional can create checklist' })
...
async function updateChecklist(req, res, sql, checklistId, userId, userType) {
  ...
  const booking = await sql`SELECT * FROM bookings WHERE id=${checklists[0].booking_id}`
  if (!booking.length) return res.status(404).json({ success: false, message: 'Booking not found' })
  if (userType !== 'professional' || booking[0].professional_id !== userId) return res.status(403).json({ success: false, message: 'Only assigned professional can update checklist' })
...
async function createReschedule(req, res, sql, userId, userType) {
  ...
  if (!booking_id || !new_date || !new_time) return res.status(400).json({ success: false, message: 'booking_id, new_date, and new_time are required' })
  const booking = await sql`SELECT * FROM bookings WHERE id=${booking_id}`
  if (!booking.length) return res.status(404).json({ success: false, message: 'Booking not found' })
  const b = booking[0]
  ...
  if (!['pending','accepted','on_the_way','arrived'].includes(b.status)) return res.status(400).json({ success: false, message: `Cannot reschedule booking with status: ${b.status}` })
  const existing = await sql`SELECT * FROM reschedule_requests WHERE booking_id=${booking_id} AND status='pending'`
  if (existing.length) return res.status(400).json({ success: false, message: 'A reschedule request is already pending' })
  ...
  if (otherUserId) await createNotification({ user_id: otherUserId, type: 'reschedule_requested', title: 'Reschedule Requested', body: `A reschedule request has been made for booking ${b.booking_number}`, data: JSON.stringify({ reschedule_id: rescheduleId, booking_id }) })
...
async function respondReschedule(req, res, sql, rescheduleId, userId) {
  ...
  if (!action || !['approve','reject'].includes(action)) return res.status(400).json({ success: false, message: 'Valid action required (approve or reject)' })
  ...
  if (!booking.length) return res.status(404).json({ success: false, message: 'Booking not found' })
  const b = booking[0]
  if (b.consumer_id !== userId && b.professional_id !== userId) return res.status(403).json({ success: false, message: 'Not authorized' })
  if (action === 'approve') {
