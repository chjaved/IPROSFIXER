...
const { hashPassword, verifyPassword, generateToken, authMiddleware } = require('../middleware/auth.js');
...
// POST /api/auth/register-pro - Professional registration
router.post('/register-pro', async (req, res) => {
  try {
    ...
    console.error('Register pro error:', err);
    return res.status(500).json({ success: false, message: err.message || 'Server error' });
  }
});
